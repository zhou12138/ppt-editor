...
> 方法：每 backend 独立子进程，冷启动 PPT，15s 间隔，warmup 2 轮，测量 5 轮  
> 状态：**5/5 backend 全部成功**
...
```
VBA inspect 170ms   = App.Run 6ms + shape遍历 20ms + VBA JSON拼接 144ms (85%)
Py Add-in    29ms   = COM bridge 5ms + shape遍历+json.dumps 24ms
```

VBA 在写操作（modify_font 20ms, set_notes 19ms）上与 Python Add-in 持平或更快，因为写操作不需要 JSON 序列化。**如果 VBA 有 `json.dumps` 级别的序列化能力，inspect 可以降到 ~30ms**，与 Python Add-in 持平。
